...
const OperationModal: React.FC<OperationModalProps> = ({ item, cars, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md">
      <div className="bg-white rounded-2xl w-full max-w-lg p-6 shadow-md animate-scaleIn relative overflow-hidden">
        ...
        <h2 className="text-2xl font-semibold text-slate-900 mb-8 uppercase tracking-tight">Детали операции</h2>

        <div className="space-y-4">
          <div className="flex items-center space-x-6">
            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-2xl ${isRental ? 'bg-blue-50 text-blue-600' : 'bg-slate-50 text-slate-600'}`}>
              <i className={`fas ${isRental ? 'fa-file-contract' : 'fa-receipt'}`}></i>
            </div>
            <div>
              <div className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide">ID Операции</div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-slate-50 p-5 rounded-2xl">
              <div className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide mb-1">Сумма</div>
              <div className={`text-xl font-bold ${!isRental && item.type === TransactionType.EXPENSE ? 'text-rose-600' : 'text-emerald-600'}`}>
                {item.totalAmount?.toLocaleString() || item.amount?.toLocaleString()} ₽
              </div>
            </div>
            <div className="bg-slate-50 p-5 rounded-2xl">
              <div className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide mb-1">Дата</div>
          ...
          <div className="bg-slate-50 p-5 rounded-2xl">
            <div className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide mb-1">Описание</div>
          ...
          {!isRental && (
             <div className="flex items-center space-x-2">
               <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide">Категория:</span>
               <span className="px-3 py-1 bg-slate-100 rounded-full text-[10px] font-semibold text-slate-600 uppercase tracking-tight">{item.category}</span>
             </div>
          )}
        </div>
