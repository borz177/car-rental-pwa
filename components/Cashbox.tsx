    ...
    <div className="space-y-5 animate-fadeIn pb-24 md:pb-0">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-400 font-bold mt-1 uppercase text-[10px] tracking-wide hidden md:block">Обзор кассы вашей компании</p>
        </div>
          ...
          {([['TODAY', 'Сегодня'], ['WEEK', '7 дней'], ['MONTH', '30 дней'], ['ALL', 'Всё время']] as const).map(([id, label]) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-[10px] font-semibold uppercase tracking-wide transition-colors ${
                period === id ? 'bg-slate-800 text-white' : 'bg-white border border-slate-100 text-slate-400 hover:text-slate-600'
              }`}
            >
              {label}
            </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-100">
          ...
          <div className={`text-3xl font-bold mt-1 ${profit >= 0 ? 'text-slate-900' : 'text-rose-600'}`}>{profit.toLocaleString()} ₽</div>
          <div className="text-[10px] font-medium text-slate-400 mt-0.5">{periodLabels[period]}</div>
        </div>
        <div className="bg-white p-5 rounded-2xl border border-slate-100 flex items-center justify-center gap-6">
          <div className="text-center">
            <div className="text-[10px] font-semibold text-emerald-500 uppercase tracking-wide">Доход</div>
            <div className="text-xl font-bold text-emerald-600 mt-1">+{incomeTotal.toLocaleString()} ₽</div>
          </div>
          <div className="w-px h-10 bg-slate-100"></div>
          <div className="text-center">
            <div className="text-[10px] font-semibold text-rose-500 uppercase tracking-wide">Расход</div>
            <div className="text-xl font-bold text-rose-600 mt-1">−{expenseTotal.toLocaleString()} ₽</div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <button
          ...
          className="group bg-white p-6 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md hover:border-emerald-200 transition-all text-left flex items-center gap-4"
        >
          <div className="w-14 h-14 rounded-2xl bg-emerald-100 text-emerald-600 flex items-center justify-center text-2xl shrink-0 group-hover:scale-105 transition-transform">
            <i className="fas fa-arrow-down"></i>
          </div>
          <div className="flex-1 min-w-0">
            <div className="font-semibold text-slate-900 text-lg">Приход</div>
            <div className="text-xs text-slate-400 font-medium mt-0.5">{periodIncome.length} операций · {periodLabels[period]}</div>
            <div className="text-emerald-600 font-bold mt-1">+{incomeTotal.toLocaleString()} ₽</div>
          </div>
        ...
        <button
          ...
          className="group bg-white p-6 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md hover:border-rose-200 transition-all text-left flex items-center gap-4"
        >
          <div className="w-14 h-14 rounded-2xl bg-rose-100 text-rose-600 flex items-center justify-center text-2xl shrink-0 group-hover:scale-105 transition-transform">
            <i className="fas fa-arrow-up"></i>
          </div>
          <div className="flex-1 min-w-0">
            <div className="font-semibold text-slate-900 text-lg">Расход</div>
            <div className="text-xs text-slate-400 font-medium mt-0.5">{periodExpenses.length} операций · {periodLabels[period]}</div>
            <div className="text-rose-600 font-bold mt-1">−{expenseTotal.toLocaleString()} ₽</div>
          </div>
          <i className="fas fa-chevron-right text-slate-300 group-hover:text-rose-500 transition-colors"></i>
        </button>
      </div>
        ...
        <div className="divide-y divide-slate-50">
          ...
          {recentActivity.length === 0 && (
            <div className="p-12 text-center">
              ...
              <div className="font-semibold text-slate-500">Операций пока нет</div>
            </div>
          )}
        </div>
      </div>
