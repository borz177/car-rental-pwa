      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
          ...
          return (
            <div key={m.id} className={`flex ${mine ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[75%] px-4 py-2.5 rounded-2xl ${
                ...
                {m.isBroadcast && (
                  <div className={`text-[9px] font-bold uppercase tracking-wide mb-1 ${mine ? 'text-blue-200' : 'text-slate-400'}`}>
                    Всем владельцам
                  </div>
                )}
                <div className="text-sm whitespace-pre-wrap break-words">{m.body}</div>
                <div className={`text-[10px] mt-1 text-right ${mine ? 'text-blue-200' : 'text-slate-400'}`}>{timeLabel(m.createdAt)}</div>
              </div>
            </div>
          );
        })}
        {messages.length === 0 && (
          <div className="h-full flex flex-col items-center justify-center text-center py-12">
            <i className="fas fa-comments text-3xl text-slate-200 mb-3"></i>
            <div className="text-sm font-semibold text-slate-400">Сообщений пока нет</div>
          </div>
        )}
        <div ref={bottomRef} />
      </div>

      <form onSubmit={handleSend} className="p-3 border-t border-slate-100 flex items-center gap-2 flex-shrink-0">
        <input
          value={text}
          onChange={e => setText(e.target.value)}
          placeholder={placeholder}
          className="flex-1 px-4 py-3 bg-slate-50 rounded-xl text-sm font-medium outline-none border-2 border-transparent focus:border-blue-500 transition-all"
        />
        <button
          type="submit"
  ...
  if (!isSuperadmin) {
    return (
      <div className="animate-fadeIn h-[calc(100vh-11rem)] md:h-[calc(100vh-8rem)]">
        <div className="bg-white rounded-2xl border border-slate-100 h-full flex flex-col overflow-hidden">
          <div className="px-4 py-3 border-b border-slate-100 flex items-center gap-3 flex-shrink-0">
            <div className="w-10 h-10 rounded-xl bg-indigo-100 text-indigo-600 flex items-center justify-center">
              <i className="fas fa-headset"></i>
            </div>
            <div>
              <div className="font-semibold text-slate-900">Техподдержка</div>
              <div className="text-[11px] text-slate-400 font-medium">Обычно отвечаем в течение дня</div>
            </div>
          </div>
          <MessageThread
            messages={messages}
  ...
  return (
    <div className="animate-fadeIn space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-semibold text-slate-900">Поддержка</h2>
          <p className="text-slate-400 font-semibold uppercase text-[10px] tracking-wide mt-0.5">Переписка с владельцами автопарков</p>
        </div>
        <button
      ...
      <div className="bg-white rounded-2xl border border-slate-100 h-[calc(100vh-14rem)] md:h-[calc(100vh-11rem)] flex overflow-hidden">
        {/* Список тредов */}
        <div className={`w-full md:w-80 border-r border-slate-100 flex-shrink-0 overflow-y-auto ${selectedAdminId ? 'hidden md:block' : ''}`}>
          {threads.map(t => (
            <button
              key={t.userId}
              onClick={() => setSelectedAdminId(t.userId)}
              className={`w-full px-4 py-3 flex items-center gap-3 text-left border-b border-slate-50 hover:bg-slate-50 transition-colors ${
                selectedAdminId === t.userId ? 'bg-blue-50' : ''
              }`}
            >
              <div className="w-10 h-10 rounded-xl bg-slate-100 flex items-center justify-center text-slate-500 font-semibold uppercase flex-shrink-0">
                {t.name.charAt(0)}
              </div>
              <div className="min-w-0 flex-1">
                ...
                <div className="text-xs text-slate-400 truncate">{t.lastMessage || 'Нет сообщений'}</div>
              </div>
            </button>
          ))}
          {threads.length === 0 && (
            <div className="p-8 text-center text-sm font-medium text-slate-400">Владельцев автопарков пока нет</div>
          )}
        </div>
        ...
        <div className={`flex-1 flex-col ${selectedAdminId ? 'flex' : 'hidden md:flex'}`}>
          {selectedThread ? (
            <>
              <div className="px-4 py-3 border-b border-slate-100 flex items-center gap-3 flex-shrink-0">
                <button onClick={() => setSelectedAdminId(null)} className="md:hidden p-1 -ml-1 text-slate-400">
                  <i className="fas fa-arrow-left"></i>
                </button>
                <div className="w-9 h-9 rounded-xl bg-slate-100 flex items-center justify-center text-slate-500 font-semibold uppercase">
                  {selectedThread.name.charAt(0)}
                </div>
                <div className="min-w-0">
                  <div className="font-semibold text-slate-900 text-sm truncate">{selectedThread.name}</div>
                  <div className="text-[11px] text-slate-400 truncate">{selectedThread.email}</div>
                </div>
              </div>
              <MessageThread
                messages={messages}
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center p-8">
              <i className="fas fa-comments text-3xl text-slate-200 mb-3"></i>
              <div className="text-sm font-semibold text-slate-400">Выберите переписку слева</div>
            </div>
          )}
        </div>
      </div>
      ...
      {showBroadcast && (
        <div className="fixed inset-0 z-[90] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <form onSubmit={handleBroadcast} className="bg-white rounded-2xl w-full max-w-md p-6 shadow-lg animate-scaleIn">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-11 h-11 rounded-2xl bg-blue-100 text-blue-600 flex items-center justify-center">
                <i className="fas fa-bullhorn"></i>
              </div>
              <div>
                <h3 className="font-semibold text-lg text-slate-900">Сообщение всем</h3>
                <p className="text-xs text-slate-400">Получат все владельцы автопарков ({threads.length})</p>
              </div>
            </div>
            <textarea
              value={broadcastText}
              ...
              rows={4}
              autoFocus
              className="w-full p-3 bg-slate-50 rounded-xl text-sm font-medium outline-none border-2 border-transparent focus:border-blue-500 transition-all resize-none"
            />
            <div className="flex gap-2 mt-4">
              <button type="button" onClick={() => setShowBroadcast(false)} className="flex-1 py-3 bg-slate-100 text-slate-600 rounded-xl font-semibold text-xs uppercase tracking-wide">
                Отмена
              </button>
