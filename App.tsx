...
const App: React.FC = () => {
 ...
 useEffect(() => {
  ...
  // fetch() не имеет таймаута по умолчанию: на нестабильной мобильной сети запрос
  // может зависнуть без ошибки на неопределённое время, и весь finally() выше просто
  // никогда не выполнится — экран загрузки виснет навсегда без объяснений. Эта
  // страховка гарантированно снимает сплеш через 12с, даже если init() всё ещё
  // ждёт сеть; когда он всё же ответит (успехом или ошибкой), состояние обновится
  // как обычно поверх уже показанного экрана.
  const safetyTimer = setTimeout(() => setIsInitializing(false), 12000);
